...
void CParser::Read_Map_Dat(LEVEL eLevel, const _string pFilePath)
{
	...
	while (File.read(reinterpret_cast<char*>(&NameLength), sizeof(_uint)))
	{
		...
		File.read(reinterpret_cast<char*>(&Desc.eObjectParkourFlag), sizeof(PARKOUR_FLAG));
		_float4x4 Matrix = {};
		...
		m_pGameInstance->Add_Work([&, ModelName = string(Desc.ModelName), ShaderPass = Desc.iShaderPassIndex, eObjectType = Desc.eObjectType,
			eParkourFlag = Desc.eObjectParkourFlag, Matrix = *Desc.WorldMatrix, BoundingPos = Desc.vBoundingPos, BoundingExtends = Desc.vBoundingExtends]() mutable {
			...
			pDesc.eObjectParkourFlag = eParkourFlag;
